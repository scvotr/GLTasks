...
const saveAndConvert = async (file, rootCustomFolder, taskFolderName) => {
  try {
    ...
    // const writeStream = fs.createWriteStream(file.mimetype === 'image/jpeg' ? newFilePath : fileIsPdf)
    const writeStream = fs.createWriteStream(file.mimetype === 'image/jpeg' || file.mimetype === 'image/png' ? newFilePath : fileIsPdf);
    ...
    return {
      fileName: file.mimetype === 'image/jpeg' || file.mimetype === 'image/png' ? fileName : file.originalFilename, // fileName: file.mimetype.toString() === 'application/pdf' ? file.originalFilename : fileName,
      thumbnailFileName,
      compersFileName,
    }
  } catch (error) {
