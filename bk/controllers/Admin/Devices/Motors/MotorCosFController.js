...
class MotorCosFController extends BaseMotorController {
  constructor() {
    super('MotorCosFT') // Передаем имя таблицы в базовый класс
  }
  async doSomething(req, res) {
    ...
    //   await this.crud.createQ(data, { checkForExist: 'name' })
      sendResponseWithData(res, 'MotorCosFController -create-ok')
    } catch (error) {
      handleError(res, 'Error: create MotorCosF - ' + error.message)
    }
  }
}

module.exports = new MotorCosFController()
