...
const createPowerRangeTable = async () => {
  try {
    //! Удаляем таблицу, если она существует
     await executeDatabaseQueryAsync(`DROP TABLE IF EXISTS powerRange`, [])

    // await executeDatabaseQueryAsync(
    //   `CREATE TABLE IF NOT EXISTS powerRange  (
    ...
    // const rows = await executeDatabaseQueryAsync('SELECT COUNT(*) as count FROM powerRange')
    // if (rows[0].count === 0) {
    //   await executeDatabaseQueryAsync(
    //     `INSERT INTO powerRange (name) VALUES 
    //       (0.37), 
    //       (0.55), 
    //       (0.75), 
    //       (1.1), 
