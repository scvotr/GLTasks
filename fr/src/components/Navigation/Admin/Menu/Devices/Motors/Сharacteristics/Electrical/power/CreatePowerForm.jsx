...
export const CreatePowerForm = ({ onClose, isEdit, dataToEdit }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const endpoint = isEdit ? "/admin/devices/motor/electrical/power/update" : "/admin/devices/motor/electrical/power/create";
      await getDataFromEndpoint(currentUser.token, endpoint, "POST", formData, setReqStatus)
      ...
      <Box sx={{ width: "100%" }}>
        ...
        >
          <FormControl>
            <TextField
              sx={{ p: 2 }}
              helperText={formData.name.length === 0 ? "Поле не может быть пустым. ФОРМАТ: 0.0 0.00" : ""}
              name="name"
              required
              type="number"
